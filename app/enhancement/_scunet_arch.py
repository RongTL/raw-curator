"""Vendored SCUNet architecture from https://github.com/cszn/SCUNet (MIT).

`basicsr` 1.4 does not ship `scunet_arch`, so we keep a local copy to avoid
adding a fragile dependency on a third-party fork. Trimmed: the upstream
`thop` import and `__main__` smoke test are removed; behaviour is unchanged.
"""
